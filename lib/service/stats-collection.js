...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'cactus-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('cactus').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('cactus')[service]);
